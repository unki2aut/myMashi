...
case class AtomSource(val url: String, private val node: Node) extends Source {
  def listItems: Seq[Item] = (node \ "entry").map(new AtomItem(_, url, image))

  val title: String = node \ "title" text
  val updated: Date =  Source.toDate(node \ "updated")
